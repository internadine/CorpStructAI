      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-white">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Verwalten Sie verschiedene Szenarien und Mandanten.</p>
          </div>
          ...
          {!isCreating ? (
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setIsCreating(false)} className="text-sm text-slate-500 hover:text-slate-700">Abbrechen</button>
            </div>
          )}
        </div>
        ...
        {/* Creation Form */}
        {isCreating && (
          <form onSubmit={handleCreateSubmit} className="p-4 bg-indigo-50 border-b border-indigo-100 flex gap-2">
            <input 
              autoFocus
              type="text"
              placeholder="Projektname eingeben..."
              className="flex-1 px-4 py-2 rounded-lg border border-indigo-200 focus:ring-2 focus:ring-indigo-500 outline-none text-sm bg-white text-slate-900"
        ...
        {/* Project List */}
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50 grid grid-cols-1 sm:grid-cols-2 gap-4">
          {projects.map(project => (
            <div 
              key={project.id}
              onClick={() => onSelectProject(project.id)}
              className={`relative group p-4 rounded-xl border-2 cursor-pointer transition-all hover:shadow-md flex flex-col ${
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                  activeProjectId === project.id ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-500'
                }`}>
              ...
              {editingProjectId === project.id ? (
                <form onSubmit={saveEditing} onClick={e => e.stopPropagation()} className="mb-1">
                  <input 
                    autoFocus
                    className="w-full border border-blue-400 rounded px-1 py-0.5 text-sm font-bold text-slate-800 outline-none bg-white"
                    value={editName}
        ...
        <div className="p-4 border-t border-slate-200 bg-white flex justify-end">
          <button 
            onClick={onClose}
            className="px-5 py-2 text-slate-600 text-sm font-medium hover:bg-slate-100 rounded-lg transition-colors"
          >
            Schließen
          </button>
        </div>
